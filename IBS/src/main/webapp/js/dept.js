...
window.methods = {
    ...
    edit: function () {
        ...
        if (rows.length == 0) {
            $.messager.alert('错误', "请选择你要编辑的dept！", "error");
            return;
        }
        if (rows.length > 1) {
        ...
        var row = rows[0];
        $.getJSON("/dept/findOne", {id: row.id}, function (data) {
            //表单回填
            $("#ffff").form("load", data);
            //弹出模态窗口
            $('#win').window('open');  // open a window
    ...
    remove: function () {
        ...
        var rows = $("#dg").datagrid("getSelections");
        if (rows.length == 0) {
            $.messager.alert('错误', "请选择你要删除的dept！", "error");
            return;
        }
        var ids = [];
        ...
        $.messager.confirm('确认', '您确认想要删除记录吗？', function (r) {
            if (r) {
                $.getJSON("/dept/delete", {ids: ids.join(",")}, function (data) {
                    if (data.status == 200) {
    ...
    submit: function () {
        ...
        $.postJSON("/dept/save", params, function (data) {
            if (data.status == 200) {
                $.messager.alert('消息', data.msg, "info");
                //关闭模态窗口
