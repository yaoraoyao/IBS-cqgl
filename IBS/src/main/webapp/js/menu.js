$(function () {
    $("#dg").datagrid({
        ...
        columns: [[
            ...
            {field: 'url', title: '资源路径', width: '10%', align: 'center'},
            {field: 'icon', title: '图标', width: '10%', align: 'center'},
            {field: 'parent_id', title: '父级菜单', width: '10%', align: 'center', hidden: true},
            {field: 'parentName', title: '父级菜单', width: '10%', align: 'center'}
        ]]
    });
...
window.methods = {
    ...
    edit: function () {
        ...
        if (rows.length == 0) {
            $.messager.alert('错误', "请选择你要编辑的菜单！", "error");
            return;
        }
        if (rows.length > 1) {
        ...
        var row = rows[0];
        $.getJSON("/menu/findOne", {id: row.id}, function (data) {
            //表单回填
            $("#ffff").form("load", data);
    ...
    remove: function () {
        ...
        $.messager.confirm('确认', '您确认想要删除记录吗？', function (r) {
            if (r) {
                $.getJSON("/menu/delete", {ids: ids.join(",")}, function (data) {
                    if (data.status == 200) {
    ...
    submit: function () {
        ...
        $.postJSON("/menu/save", params, function (data) {
            if (data.status == 200) {
                $.messager.alert('消息', data.msg, "info");
                //关闭模态窗口
