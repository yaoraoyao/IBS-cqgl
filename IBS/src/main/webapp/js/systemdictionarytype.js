...
window.methods = {
    ...
    edit: function () {
        ...
        var row = rows[0];
        $.getJSON("/systemDictionaryType/findOne", {id: row.id}, function (data) {
            //表单回填
            $("#ffff").form("load", data);
            //弹出模态窗口
            $('#win').window('open');  // open a window
    ...
    remove: function () {
        ...
        $.messager.confirm('确认', '您确认想要删除记录吗？', function (r) {
            if (r) {
                $.getJSON("/systemDictionaryType/delete", {ids: ids.join(",")}, function (data) {
                    if (data.status == 200) {
    ...
    submit: function () {
        ...
        $.postJSON("/systemDictionaryType/save", params, function (data) {
            if (data.status == 200) {
                $.messager.alert('消息', data.msg, "info");
                //关闭模态窗口
