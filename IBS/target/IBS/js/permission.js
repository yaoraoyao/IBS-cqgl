$(function () {
    $("#dg").datagrid({
        ...
        columns: [[
            ...
            {field:'sn',title:'权限编码',width:'10%',align:'center'},
            {field:'menu',title:'菜单',width:'10%',align:'center',formatter:formatObj},
        ]]
    });
...
window.methods = {
    ...
    edit: function () {
        ...
        var row = rows[0];
        $.getJSON("/permission/findOne", {id: row.id}, function (data) {
            //表单回填
            $("#ffff").form("load", data);
            if (data.menu==null){
    ...
    remove: function () {
        ...
        $.messager.confirm('确认', '您确认想要删除记录吗？', function (r) {
            if (r) {
                $.getJSON("/permission/delete", {ids: ids.join(",")}, function (data) {
                    if (data.status == 200) {
    ...
    submit: function () {
        ...
        $.postJSON("/permission/save", params, function (data) {
            if (data.status == 200) {
                $.messager.alert('消息', data.msg, "info");
                //关闭模态窗口
