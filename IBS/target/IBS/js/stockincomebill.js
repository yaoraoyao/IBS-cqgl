$(function () {
    $("#dg").datagrid({
        ...
        columns: [[
            ...
            {field:'vdate',title:'送货日期',width:'10%',align:'center'},
            ...
            {field:'keeper',title:'库管员',width:'10%',align:'center',formatter:formatEmp},
            {field:'depot',title:'仓库',width:'10%',align:'center',formatter:formatObj},
    ...
    var buttonBindEvent = function () {
        ...
        $("#btnAudite").click(function () {
            ...
            $.postJSON("/stockincomebill/audite",{id: id}, function (data) {
                if (data.status == 200) {
...
window.methods = {
    ...
    edit: function () {
        ...
        var row = rows[0];
        $.getJSON("/stockincomebill/findOne", {id: row.id}, function (data) {
            if(data.status==1){
                $.messager.alert('错误', "无法操作已经审核的入库单！", "error");
                return;
            }
            //表单回填
            $("#ffff").form("load", data);
            $("#supplierId").combobox("setValue",data.supplier.id);
            $("#keeperId").combobox("setValue",data.keeper.id);
            $("#depotId").combobox("setValue",data.depot.id);
            //回填一下采购单明细datagrid
    ...
    remove: function () {
        ...
        $.messager.confirm('确认', '您确认想要删除记录吗？', function (r) {
            if (r) {
                $.getJSON("/stockincomebill/delete", {ids: ids.join(",")}, function (data) {
                    if (data.status == 200) {
                        $.messager.alert('消息', data.msg, "info");
                        //重新刷新表格的数据
                        $("#dg").datagrid('load', {
    ...
    submit: function () {
        ...
        $.postJSON("/stockincomebill/save", params, function (data) {
            if (data.status == 200) {
                $.messager.alert('消息', data.msg, "info");
                //关闭模态窗口
                $('#win').window('close');  // close a window
